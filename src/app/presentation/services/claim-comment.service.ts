...
export class ClaimCommentService {
  ...
  public async registerClaimComment(
    newClaimComment: RegisterClaimCommentDTO,
  ...
  private async validateNewClaimComment(
    newClaimComment: RegisterClaimCommentDTO,
  ) {
    if (
      newClaimComment.role == Role.COMMUNITY &&
      !(await this.existCommunityWithClaim(
        newClaimComment.entityId,
        newClaimComment.claimId,
      ))
    )
      throw new BadRequestException(
        'The Claim does not exist in this Community!',
      );
    else if (
      newClaimComment.role == Role.USER &&
      !(await this.existUserWithClaim(
        newClaimComment.entityId,
        newClaimComment.claimId,
      ))
    )
  ...
  public async getClaimComments(id: any) {
    return await this.claimCommentRepository.find({
      where: { claimId: id },
      order: { date: 'ASC' },
    });
  }
  ...
  private async existUserWithClaim(entityId: string, claimId: string) {
    return !!(await this.claimRepository.findOne({
      where: { id: claimId, userId: entityId },
    }));
  }
  private async existCommunityWithClaim(entityId: string, claimId: string) {
    return !!(await this.claimRepository.findOne({
      where: { id: claimId, communityId: entityId },
    }));
  }
}
