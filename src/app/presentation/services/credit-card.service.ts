...
export class CreditCardService {
  ...
  public async registerCreditCard(newCreditCard: RegisterCreditCardDTO): Promise<CreditCard> {
  ...
  private async existsCreditCardWithNumber(creditCardNumber: string) {
    return !! await this.creditCardRepository.findOne({ where: {number: creditCardNumber} });
  }
